/**
 * Google Apps Script untuk Mahasiswa Feedback Platform - COMPLETE VERSION
...
function testConnection() {
  return {
    ...
    cors_enabled: true,
    drive_folder_configured: DRIVE_FOLDER_ID ? true : false
  };
}
      ...
      if (userData.length > 1 && post.userId) {
        for (var j = 1; j < userData.length; j++) {
          if (userData[j][0] === post.userId) {
            post.username = userData[j][2] || "Anonymous";
            break;
          }
        }
      }
  ...
  try {
    ...
    for (var i = 1; i < postData.length; i++) {
      if (postData[i][0] === postId) {
        var currentLikes = parseInt(postData[i][6] || 0);
        var currentDislikes = parseInt(postData[i][7] || 0);
...
function handleUpdateProfile(e) {
  try {
    ...
    for (var i = 1; i < userData.length; i++) {
      if (userData[i][0] === userId) {
        if (data.username) usersSheet.getRange(i + 1, 3).setValue(data.username);
        if (data.nim) usersSheet.getRange(i + 1, 5).setValue(data.nim);
        if (data.gender) usersSheet.getRange(i + 1, 6).setValue(data.gender);
        if (data.jurusan) usersSheet.getRange(i + 1, 7).setValue(data.jurusan);
  ...
  try {
    ...
    if (postingSheet) {
      ...
      if (postData.length > 1) {
        ...
        for (var i = 1; i < postData.length; i++) {
          stats.totalLikes += parseInt(postData[i][6] || 0);
          stats.totalDislikes += parseInt(postData[i][7] || 0);

          var postDate = new Date(postData[i][5]).toDateString();
          if (postDate === today) {
            stats.todayPosts++;
          }
        }
...
function verifyHashedPassword(plainPassword, hashedPassword) {
  if (hashedPassword.indexOf('$2') === 0) {
    return false; // Can't verify bcrypt in Google Apps Script
  }

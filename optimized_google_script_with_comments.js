...
function testConnection() {
  return {
    message: "Connection successful - OPTIMIZED VERSION",
    timestamp: new Date().toISOString(),
    status: "ok",
    features: ["posts", "comments", "likes", "upload"],
    performance: "optimized"
  };
}

// OPTIMIZED: Get sheet with caching
function getSheet(sheetName) {
  var cacheKey = "sheet_" + sheetName;
  if (cache[cacheKey] && cache[cacheKey].timestamp > Date.now() - (CACHE_DURATION * 1000)) {
...
// OPTIMIZED: Get posts with better performance
...
// OPTIMIZED: Like/Dislike with reduced operations
...
// OPTIMIZED: Update post (preserve original timestamp)
function handleUpdatePost(e) {
  try {
    ...
    for (var i = 1; i < postData.length; i++) {
      if (postData[i][0] === postId && postData[i][1] === userId) {
        if (judul !== undefined) {
...
// Other functions remain similar but optimized...
